...
public class DatabaseSSHtunneling{
      ...
      try{
          ...
          switch(selection){
            ...
            case 5: //Visualize the number of drills per coach
              start = System.currentTimeMillis();
              rs = stmt.executeQuery("SELECT coach.name, coach.surname, COUNT(*) AS FROM coach INNER JOIN drill ON drill.usernamecoach = coach.username GROUP BY coach.username");
              end = System.currentTimeMillis();
              end-=start;
              System.out.format("%S%d%s\n","Query completely executed in ",end," ms");
